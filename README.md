...
**Engine, API and UI are built and measured.** 616 tests green. The full pipeline runs
...
structurally cannot produce (ADR-031, ADR-033, ADR-037 – ADR-044). That story is in
...
UI renders one hardcoded batch, and a merchant whose CSV uses unfamiliar column names
gets a correct 422 with no way to act on it from the browser — the column-mapping picker
is not built. All are stated in [docs/LIMITATIONS.md](docs/LIMITATIONS.md).
